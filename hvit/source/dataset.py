...
def get_slide2vec_feature_stem(path: str) -> str:
    return Path(path).stem.replace(" ", "_")


def get_feature_stem_candidates(row) -> list[str]:
    candidates = []
    for column in ["slide_id", "wsi_path", "slide_path"]:
        if column not in row or pd.isna(row[column]):
            continue
        value = str(row[column])
        if column == "slide_id":
            candidates.append(value)
        candidates.append(get_slide2vec_feature_stem(value))
    return list(dict.fromkeys(candidates))


def resolve_feature_stem(features_dir: Path, row) -> Optional[str]:
    candidates = get_feature_stem_candidates(row)
    for stem in candidates:
        if Path(features_dir, f"{stem}.pt").is_file():
            return stem
...
class ExtractedFeaturesDataset(torch.utils.data.Dataset):
    ...
    def prepare_data(self, df):
        ...
        df["_feature_stem"] = df.apply(
            lambda row: resolve_feature_stem(self.features_dir, row), axis=1
        )
        has_features = df["_feature_stem"].apply(
            lambda stem: stem is not None
            and Path(self.features_dir, f"{stem}.pt").is_file()
        )
        df_missing = df[~has_features].reset_index(drop=True)
        df_filtered = df[has_features].reset_index(drop=True)
    ...
    def __getitem__(self, idx: int):
        ...
        feature_stem = getattr(row, "_feature_stem", slide_id)
        fp = Path(self.features_dir, f"{feature_stem}.pt")
