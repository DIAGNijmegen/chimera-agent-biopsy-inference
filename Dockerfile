...
# vendored HViT pipeline (5-fold MIL ensemble), trimmed to the inference path;
# the inference config lives at hvit/config/inference/panda-inference.yaml
COPY --chown=user:user hvit ./hvit
...
#   - hvit requirements (the base image already provides torch/torchvision/numpy)
...
# HViT model weights — per-fold DINO ViT feature extractors (pretrained/) and
...
RUN mkdir -p hvit/checkpoints/pretrained hvit/checkpoints/trained && \
    ...
    for f in 0 1 2 3 4; do \
      ...
      fetch "${base}/vit_256_small_dino_fold_${f}.pt" "hvit/checkpoints/pretrained/vit_256_small_dino_fold_${f}.pt" && \
      echo "Downloading fold-${f}.pt" && \
      fetch "${base}/fold-${f}.pt" "hvit/checkpoints/trained/fold-${f}.pt"; \
    done && \
    sha256sum -c resources/SHA256SUMS && \
    echo "all weights verified"

# vendored slide2vec/hvit on the import path (CONCH/MUSK resolve from site-packages)
ENV PYTHONPATH="/opt/app/slide2vec:/opt/app/hvit:${PYTHONPATH}"
