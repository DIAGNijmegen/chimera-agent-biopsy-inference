...
def get_wsi_name(wsi_path: Path) -> str:
    return Path(wsi_path).stem.replace(" ", "_")


def get_slides_with_tiles(wsi_paths, coordinates_dir: Path) -> list[str]:
    return [
        str(p)
        for p in wsi_paths
        if Path(coordinates_dir, f"{get_wsi_name(p)}.npy").is_file()
    ]


def mark_tiling_success_from_coordinates(process_df: pd.DataFrame, coordinates_dir: Path):
    coordinates_names = {p.stem for p in Path(coordinates_dir).glob("*.npy")}
    normalized_names = process_df["wsi_name"].astype(str).str.replace(" ", "_", regex=False)
    process_df.loc[normalized_names.isin(coordinates_names), "tiling_status"] = "success"
    return process_df
...
def process_slide(
    *,
    ...
    """
    wsi_name = get_wsi_name(wsi_path)
    try:
...
def main(args):
    ...
    if not skip_tiling:
        if cfg.tiling.read_coordinates_from is not None:
            ...
            process_df = mark_tiling_success_from_coordinates(process_df, coordinates_dir)
        ...
        slides_with_tiles = get_slides_with_tiles(wsi_paths, coordinates_dir)
