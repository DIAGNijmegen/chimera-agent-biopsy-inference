...
class WholeSlideImage(object):
    ...
    def load_segmentation(
        ...
        self.binary_mask = self._normalize_binary_mask(
            mask,
            tissue_pixel_value=segment_params.tissue_pixel_value,
            foreground_value=segment_params.sthresh_up,
        )
        return seg_level
    ...
    def _normalize_binary_mask(
        mask: np.ndarray,
        tissue_pixel_value: int,
        foreground_value: int,
    ) -> np.ndarray:
        """
        Convert a loaded segmentation mask to OpenCV contour-compatible format.
        """
        if mask.ndim == 3:
            mask = mask[:, :, 0]

        binary_mask = (mask == tissue_pixel_value).astype(np.uint8)
        return binary_mask * foreground_value
